...
- No build system should be required, ideally. However, in some cases the reality is that even without dependencies, just writing code against the native platform might require a build system to resolve the system include headers, for example Windows.h. And on the other hand the compiler might require specific include directories to even compile C++ code. In such cases, **sai** will offer build scripts for the native platform, and for CMake and other popular build systems. So that whatever build system you're using, you would not have to worry about it, and it should not be a bourden. Today, the basic idea with supporting sai in your project, is that if you have code targeting Windows, for example, and it works, then inserting **sai** into your project should be as simple as including the **sai** source files into your project, with no additional configurations required. In the long term, a more permamement solution will be required that fixes this problem.
